...
namespace AMS_MVC.Controllers
{
    ...
    {
        [HttpGet]
        public ActionResult DCCABLEBasicAdd()
        {
            ...
            return View("~/Views/Regist/DCCABLE/DCCABLEBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult DCCABLEBasicAdd(DCCABLEBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (DCCABLE_CODE 생성 등)
            DCCABLEBasicInfoRepository repo = new DCCABLEBasicInfoRepository();
            string latestCode = repo.GetLatestDCCABLECode();
            model.DCCABLE_Code = GenerateNextDCCABLECode(latestCode, "DCCABLE");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateDCCABLEBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextDCCABLECode(string latestCode, string prefix)
        {
            int codeNumber = 0;
