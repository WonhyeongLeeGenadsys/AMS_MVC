...
namespace AMS_MVC
{
    public partial class DCCABLEBasicController : Controller
    {
        ...
        [HttpGet]
        public ActionResult UpdateDCCABLEBasic(string tblIdx)
        {
            var model = dccableBasicRepository.GetDCCABLEBasicInfoByTblIdxRepo(tblIdx);
            if (model == null)
            {
            ...
            return View("~/Views/Regist/DCCABLE/DCCABLEBasicUpdate.cshtml", model);
        }

        // POST: 수정 요청 처리
        [HttpPost]
        public ActionResult UpdateDCCABLEBasicInfo(DCCABLEBasicInfo model, HttpPostedFileBase photo)
        {
            ...
            var result = dccableBasicRepository.UpdateDCCABLEBasicInfoRepo(model);
