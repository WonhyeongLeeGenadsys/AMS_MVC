...
namespace AMS_MVC
{
    public partial class SUBMODULEBasicController : Controller
    {
        ...
        [HttpGet]
        public ActionResult UpdateSUBMODULEBasic(string tblIdx)
        {
            var model = submoduleBasicRepository.GetSUBMODULEBasicInfoByTblIdxRepo(tblIdx);
            if (model == null)
            {
            ...
            return View("~/Views/Regist/SUBMODULE/SUBMODULEBasicUpdate.cshtml", model);
        }

        // POST: 수정 요청 처리
        [HttpPost]
        public ActionResult UpdateSUBMODULEBasicInfo(SUBMODULEBasicInfo model, HttpPostedFileBase photo)
        {
            ...
            var result = submoduleBasicRepository.UpdateSUBMODULEBasicInfoRepo(model);
