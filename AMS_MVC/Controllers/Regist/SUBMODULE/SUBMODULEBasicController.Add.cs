...
namespace AMS_MVC
{
    ...
    {
        [HttpGet]
        public ActionResult SUBMODULEBasicAdd()
        {
            ...
            return View("~/Views/Regist/SUBMODULE/SUBMODULEBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult SUBMODULEBasicAdd(SUBMODULEBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (SUBMODULE_CODE 생성 등)
            SUBMODULEBasicInfoRepository repo = new SUBMODULEBasicInfoRepository();
            string latestCode = repo.GetLatestSUBMODULECode();
            model.SUBMODULE_Code = GenerateNextSUBMODULECode(latestCode, "SUBMODULE");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateSUBMODULEBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextSUBMODULECode(string latestCode, string prefix)
        {
            int codeNumber = 0;
