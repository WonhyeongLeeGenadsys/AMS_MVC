...
namespace AMS_MVC.Controllers
{
    ...
    {
        [HttpGet]
        public ActionResult ITRBasicUpdate(string tblIdx)
        {
            var model = itrBasicRepository.GetITRBasicInfoByTblIdxRepo(tblIdx);
            if(model == null)
            {
                return HttpNotFound("InterfaceTR 기본정보를 찾을 수 없습니다.");
            }
            ...
            return View("~/Views/Regist/ITR/ITRBasicUpdate.cshtml", model);
        }

        [HttpPost]
        public ActionResult ITRBasicUpdate(ITRBasicInfo model, HttpPostedFileBase photo)
        {
            ...
            var result = itrBasicRepository.UpdateITRBasicInfoRepo(model);
