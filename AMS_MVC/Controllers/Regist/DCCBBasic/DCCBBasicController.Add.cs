...
namespace AMS_MVC
{
    ...
    {
        [HttpGet]
        public ActionResult DCCBBasicAdd()
        {
            ...
            return View("~/Views/Regist/DCCB/DCCBBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult DCCBBasicAdd(DCCBBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            DCCBBasicInfoRepository repo = new DCCBBasicInfoRepository();
            string latestCode = repo.GetLatestDCCBCode();
            model.DCCB_Code = GenerateNextDCCBCode(latestCode, "DCCB");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateDCCBBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextDCCBCode(string latestCode, string prefix)
        {
            int codeNumber = 0;
