...
namespace AMS_MVC
{
    public partial class PUMPBasicController : Controller
    {
        ...
        [HttpGet]
        public ActionResult UpdatePUMPBasic(string tblIdx)
        {
            var model = pumpBasicRepository.GetPUMPBasicInfoByTblIdxRepo(tblIdx);
            if (model == null)
            {
            ...
            return View("~/Views/Regist/etc/Cooling System/PUMP/PUMPBasicUpdate.cshtml", model);
        }

        // POST: 수정 요청 처리
        [HttpPost]
        public ActionResult UpdatePUMPBasicInfo(PUMPBasicInfo model, HttpPostedFileBase photo)
        {
            ...
            var result = pumpBasicRepository.UpdatePUMPBasicInfoRepo(model);
