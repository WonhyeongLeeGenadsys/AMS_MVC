...
namespace AMS_MVC.Controllers
{
    ...
    {
        [HttpGet]
        public ActionResult PUMPBasicAdd()
        {
            ...
            return View("~/Views/Regist/etc/Cooling System/PUMP/PUMPBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult PUMPBasicAdd(PUMPBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (PUMP_CODE 생성 등)
            PUMPBasicInfoRepository repo = new PUMPBasicInfoRepository();
            string latestCode = repo.GetLatestPUMPCode();
            model.PUMP_Code = GenerateNextPUMPCode(latestCode, "PUMP");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreatePUMPBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextPUMPCode(string latestCode, string prefix)
        {
            int codeNumber = 0;
