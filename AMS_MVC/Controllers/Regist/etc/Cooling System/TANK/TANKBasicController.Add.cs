...
namespace AMS_MVC
{
    ...
    {
        [HttpGet]
        public ActionResult TANKBasicAdd()
        {
            ...
            return View("~/Views/Regist/etc/Cooling System/TANK/TANKBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult TANKBasicAdd(TANKBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (TANK_CODE 생성 등)
            TANKBasicInfoRepository repo = new TANKBasicInfoRepository();
            string latestCode = repo.GetLatestTANKCode();
            model.TANK_Code = GenerateNextTANKCode(latestCode, "TANK");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateTANKBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextTANKCode(string latestCode, string prefix)
        {
            int codeNumber = 0;
