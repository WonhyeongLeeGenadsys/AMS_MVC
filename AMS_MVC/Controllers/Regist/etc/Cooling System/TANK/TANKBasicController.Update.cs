...
namespace AMS_MVC
{
    public partial class TANKBasicController : Controller
    {
        ...
        [HttpGet]
        public ActionResult UpdateTANKBasic(string tblIdx)
        {
            var model = tankBasicRepository.GetTANKBasicInfoByTblIdxRepo(tblIdx);
            if (model == null)
            {
            ...
            return View("~/Views/Regist/etc/Cooling System/TANK/TANKBasicUpdate.cshtml", model);
        }

        // POST: 수정 요청 처리
        [HttpPost]
        public ActionResult UpdateTANKBasicInfo(TANKBasicInfo model, HttpPostedFileBase photo)
        {
            ...
            var result = tankBasicRepository.UpdateTANKBasicInfoRepo(model);
