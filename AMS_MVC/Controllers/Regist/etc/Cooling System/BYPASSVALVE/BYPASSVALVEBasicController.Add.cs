...
namespace AMS_MVC.Controllers
{
    ...
    {
        [HttpGet]
        public ActionResult BYPASSVALVEBasicAdd()
        {
            ...
            return View("~/Views/Regist/etc/Cooling System/BYPASSVALVE/BYPASSVALVEBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult BYPASSVALVEBasicAdd(BYPASSVALVEBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (BYPASSVALVE_CODE 생성 등)
            BYPASSVALVEBasicInfoRepository repo = new BYPASSVALVEBasicInfoRepository();
            string latestCode = repo.GetLatestBYPASSVALVECode();
            model.BYPASSVALVE_Code = GenerateNextBYPASSVALVECode(latestCode, "BYPASSVALVE");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateBYPASSVALVEBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextBYPASSVALVECode(string latestCode, string prefix)
        {
            int codeNumber = 0;
