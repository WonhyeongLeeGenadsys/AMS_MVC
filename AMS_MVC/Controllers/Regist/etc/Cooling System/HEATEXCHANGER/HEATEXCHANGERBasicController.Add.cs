...
namespace AMS_MVC.Controllers
{
    ...
    {
        [HttpGet]
        public ActionResult HEATEXCHANGERBasicAdd()
        {
            ...
            return View("~/Views/Regist/etc/Cooling System/HEATEXCHANGER/HEATEXCHANGERBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult HEATEXCHANGERBasicAdd(HEATEXCHANGERBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (HEATEXCHANGER_CODE 생성 등)
            HEATEXCHANGERBasicInfoRepository repo = new HEATEXCHANGERBasicInfoRepository();
            string latestCode = repo.GetLatestHEATEXCHANGERCode();
            model.HEATEXCHANGER_Code = GenerateNextHEATEXCHANGERCode(latestCode, "HEATEXCHANGER");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateHEATEXCHANGERBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextHEATEXCHANGERCode(string latestCode, string prefix)
        {
            int codeNumber = 0;
