...
namespace AMS_MVC
{
    public partial class HEATEXCHANGERBasicController : Controller
    {
        ...
        [HttpGet]
        public ActionResult UpdateHEATEXCHANGERBasic(string tblIdx)
        {
            var model = heatexchangerBasicRepository.GetHEATEXCHANGERBasicInfoByTblIdxRepo(tblIdx);
            if (model == null)
            {
            ...
            return View("~/Views/Regist/etc/Cooling System/HEATEXCHANGER/HEATEXCHANGERBasicUpdate.cshtml", model);
        }

        // POST: 수정 요청 처리
        [HttpPost]
        public ActionResult UpdateHEATEXCHANGERBasicInfo(HEATEXCHANGERBasicInfo model, HttpPostedFileBase photo)
        {
            ...
            var result = heatexchangerBasicRepository.UpdateHEATEXCHANGERBasicInfoRepo(model);
