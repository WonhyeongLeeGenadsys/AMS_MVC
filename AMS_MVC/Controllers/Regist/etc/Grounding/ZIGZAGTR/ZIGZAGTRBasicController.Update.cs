...
namespace AMS_MVC
{
    public partial class ZIGZAGTRBasicController : Controller
    {
        ...
        [HttpGet]
        public ActionResult UpdateZIGZAGTRBasic(string tblIdx)
        {
            var model = zigzagtrBasicRepository.GetZIGZAGTRBasicInfoByTblIdxRepo(tblIdx);
            if (model == null)
            {
            ...
            return View("~/Views/Regist/etc/Grounding/ZIGZAGTR/ZIGZAGTRBasicUpdate.cshtml", model);
        }

        // POST: 수정 요청 처리
        [HttpPost]
        public ActionResult UpdateZIGZAGTRBasicInfo(ZIGZAGTRBasicInfo model, HttpPostedFileBase photo)
        {
            ...
            var result = zigzagtrBasicRepository.UpdateZIGZAGTRBasicInfoRepo(model);
