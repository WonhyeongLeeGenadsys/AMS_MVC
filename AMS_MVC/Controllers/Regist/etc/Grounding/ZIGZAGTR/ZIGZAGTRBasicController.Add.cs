...
namespace AMS_MVC.Controllers
{
    ...
    {
        [HttpGet]
        public ActionResult ZIGZAGTRBasicAdd()
        {
            ...
            return View("~/Views/Regist/etc/Grounding/ZIGZAGTR/ZIGZAGTRBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult ZIGZAGTRBasicAdd(ZIGZAGTRBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (ZIGZAGTR_CODE 생성 등)
            ZIGZAGTRBasicInfoRepository repo = new ZIGZAGTRBasicInfoRepository();
            string latestCode = repo.GetLatestZIGZAGTRCode();
            model.ZIGZAGTR_Code = GenerateNextZIGZAGTRCode(latestCode, "ZIGZAGTR");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateZIGZAGTRBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextZIGZAGTRCode(string latestCode, string prefix)
        {
            int codeNumber = 0;
