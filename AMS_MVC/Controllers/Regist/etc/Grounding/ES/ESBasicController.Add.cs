...
namespace AMS_MVC
{
    ...
    {
        [HttpGet]
        public ActionResult ESBasicAdd()
        {
            ...
            return View("~/Views/Regist/etc/Grounding/ES/ESBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult ESBasicAdd(ESBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (ES_CODE 생성 등)
            ESBasicInfoRepository repo = new ESBasicInfoRepository();
            string latestCode = repo.GetLatestESCode();
            model.ES_Code = GenerateNextESCode(latestCode, "ES");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateESBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextESCode(string latestCode, string prefix)
        {
            int codeNumber = 0;
