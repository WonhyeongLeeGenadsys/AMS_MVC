...
namespace AMS_MVC.Controllers
{
    public partial class NGRBasicController : Controller
    {
        ...
        [HttpGet]
        public ActionResult UpdateNGRBasic(string tblIdx)
        {
            var model = ngrBasicRepository.GetNGRBasicInfoByTblIdxRepo(tblIdx);
            if (model == null)
            {
            ...
            return View("~/Views/Regist/etc/Grounding/NGR/NGRBasicUpdate.cshtml", model);
        }

        // POST: 수정 요청 처리
        [HttpPost]
        public ActionResult UpdateNGRBasicInfo(NGRBasicInfo model, HttpPostedFileBase photo)
        {
            ...
            var result = ngrBasicRepository.UpdateNGRBasicInfoRepo(model);
