...
namespace AMS_MVC.Controllers
{
    ...
    {
        [HttpGet]
        public ActionResult NGRBasicAdd()
        {
            ...
            return View("~/Views/Regist/etc/Grounding/NGR/NGRBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult NGRBasicAdd(NGRBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (NGR_CODE 생성 등)
            NGRBasicInfoRepository repo = new NGRBasicInfoRepository();
            string latestCode = repo.GetLatestNGRCode();
            model.NGR_Code = GenerateNextNGRCode(latestCode, "NGR");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateNGRBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextNGRCode(string latestCode, string prefix)
        {
            int codeNumber = 0;
