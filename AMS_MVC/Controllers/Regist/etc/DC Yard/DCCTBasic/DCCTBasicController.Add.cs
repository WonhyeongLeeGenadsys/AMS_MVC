...
namespace AMS_MVC.Controllers
{
    ...
    {
        [HttpGet]
        public ActionResult DCCTBasicAdd()
        {
            ...
            return View("~/Views/Regist/etc/DC Yard/DCCT/DCCTBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult DCCTBasicAdd(DCCTBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (DCCT_CODE 생성 등)
            DCCTBasicInfoRepository repo = new DCCTBasicInfoRepository();
            string latestCode = repo.GetLatestDCCTCode();
            model.DCCT_Code = GenerateNextDCCTCode(latestCode, "DCCT");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateDCCTBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextDCCTCode(string latestCode, string prefix)
        {
            int codeNumber = 0;
