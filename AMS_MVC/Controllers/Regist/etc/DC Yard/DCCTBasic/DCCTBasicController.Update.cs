...
namespace AMS_MVC.Controllers
{
    public partial class DCCTBasicController : Controller
    {
        ...
        [HttpGet]
        public ActionResult UpdateDCCTBasic(string tblIdx)
        {
            var model = dcctBasicRepository.GetDCCTBasicInfoByTblIdxRepo(tblIdx);
            if (model == null)
            {
            ...
            return View("~/Views/Regist/etc/DC Yard/DCCT/DCCTBasicUpdate.cshtml", model);
        }

        // POST: 수정 요청 처리
        [HttpPost]
        public ActionResult UpdateDCCTBasicInfo(DCCTBasicInfo model, HttpPostedFileBase photo)
        {
            ...
            var result = dcctBasicRepository.UpdateDCCTBasicInfoRepo(model);
