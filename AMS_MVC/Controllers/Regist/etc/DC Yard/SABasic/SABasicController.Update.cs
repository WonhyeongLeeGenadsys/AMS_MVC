...
namespace AMS_MVC
{
    public partial class SABasicController : Controller
    {
        ...
        [HttpGet]
        public ActionResult UpdateSABasic(string tblIdx)
        {
            var model = saBasicRepository.GetSABasicInfoByTblIdxRepo(tblIdx);
            if (model == null)
            {
            ...
            return View("~/Views/Regist/etc/DC Yard/SA/SABasicUpdate.cshtml", model);
        }

        // POST: 수정 요청 처리
        [HttpPost]
        public ActionResult UpdateSABasicInfo(SABasicInfo model, HttpPostedFileBase photo)
        {
            ...
            var result = saBasicRepository.UpdateSABasicInfoRepo(model);
