...
namespace AMS_MVC.Controllers
{
    ...
    {
        [HttpGet]
        public ActionResult SABasicAdd()
        {
            ...
            return View("~/Views/Regist/etc/DC Yard/SA/SABasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult SABasicAdd(SABasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (SA_CODE 생성 등)
            SABasicInfoRepository repo = new SABasicInfoRepository();
            string latestCode = repo.GetLatestSACode();
            model.SA_Code = GenerateNextSACode(latestCode, "SA");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateSABasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextSACode(string latestCode, string prefix)
        {
            int codeNumber = 0;
