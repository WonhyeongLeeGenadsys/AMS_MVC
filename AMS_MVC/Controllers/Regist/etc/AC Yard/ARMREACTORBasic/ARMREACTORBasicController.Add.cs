...
namespace AMS_MVC
{
    ...
    {
        [HttpGet]
        public ActionResult ARMREACTORBasicAdd()
        {
            ...
            return View("~/Views/Regist/etc/AC Yard/ARMREACTOR/ARMREACTORBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult ARMREACTORBasicAdd(ARMREACTORBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (ARMREACTOR_CODE 생성 등)
            ARMREACTORBasicInfoRepository repo = new ARMREACTORBasicInfoRepository();
            string latestCode = repo.GetLatestARMREACTORCode();
            model.ARMREACTOR_Code = GenerateNextARMREACTORCode(latestCode, "ARMREACTOR");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateARMREACTORBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextARMREACTORCode(string latestCode, string prefix)
        {
            int codeNumber = 0;
