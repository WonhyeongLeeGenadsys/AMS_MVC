...
namespace AMS_MVC.Controllers
{
    public partial class ARMREACTORBasicController : Controller
    {
        ...
        [HttpGet]
        public ActionResult UpdateARMREACTORBasic(string tblIdx)
        {
            var model = armreactorBasicRepository.GetARMREACTORBasicInfoByTblIdxRepo(tblIdx);
            if (model == null)
            {
            ...
            return View("~/Views/Regist/etc/AC Yard/ARMREACTOR/ARMREACTORBasicUpdate.cshtml", model);
        }

        // POST: 수정 요청 처리
        [HttpPost]
        public ActionResult UpdateARMREACTORBasicInfo(ARMREACTORBasicInfo model, HttpPostedFileBase photo)
        {
            ...
            var result = armreactorBasicRepository.UpdateARMREACTORBasicInfoRepo(model);
