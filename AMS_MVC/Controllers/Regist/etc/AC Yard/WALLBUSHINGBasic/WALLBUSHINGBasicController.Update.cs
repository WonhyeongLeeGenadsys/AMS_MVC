...
namespace AMS_MVC
{
    public partial class WALLBUSHINGBasicController : Controller
    {
        ...
        [HttpGet]
        public ActionResult UpdateWALLBUSHINGBasic(string tblIdx)
        {
            var model = wallbushingBasicRepository.GetWALLBUSHINGBasicInfoByTblIdxRepo(tblIdx);
            if (model == null)
            {
            ...
            return View("~/Views/Regist/etc/AC Yard/WALLBUSHING/WALLBUSHINGBasicUpdate.cshtml", model);
        }

        // POST: 수정 요청 처리
        [HttpPost]
        public ActionResult UpdateWALLBUSHINGBasicInfo(WALLBUSHINGBasicInfo model, HttpPostedFileBase photo)
        {
            ...
            var result = wallbushingBasicRepository.UpdateWALLBUSHINGBasicInfoRepo(model);
