...
namespace AMS_MVC.Controllers
{
    ...
    {
        [HttpGet]
        public ActionResult WALLBUSHINGBasicAdd()
        {
            ...
            return View("~/Views/Regist/etc/AC Yard/WALLBUSHING/WALLBUSHINGBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult WALLBUSHINGBasicAdd(WALLBUSHINGBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (WALLBUSHING_CODE 생성 등)
            WALLBUSHINGBasicInfoRepository repo = new WALLBUSHINGBasicInfoRepository();
            string latestCode = repo.GetLatestWALLBUSHINGCode();
            model.WALLBUSHING_Code = GenerateNextWALLBUSHINGCode(latestCode, "WALLBUSHING");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateWALLBUSHINGBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextWALLBUSHINGCode(string latestCode, string prefix)
        {
            int codeNumber = 0;
