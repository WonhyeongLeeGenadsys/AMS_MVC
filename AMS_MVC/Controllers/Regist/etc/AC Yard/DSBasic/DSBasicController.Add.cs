...
namespace AMS_MVC.Controllers
{
    ...
    {
        [HttpGet]
        public ActionResult DSBasicAdd()
        {
            ...
            return View("~/Views/Regist/etc/AC Yard/DS/DSBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult DSBasicAdd(DSBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (DS_CODE 생성 등)
            DSBasicInfoRepository repo = new DSBasicInfoRepository();
            string latestCode = repo.GetLatestDSCode();
            model.DS_Code = GenerateNextDSCode(latestCode, "DS");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateDSBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextDSCode(string latestCode, string prefix)
        {
            int codeNumber = 0;
