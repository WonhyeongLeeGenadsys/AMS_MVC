...
namespace AMS_MVC
{
    ...
    {
        [HttpGet]
        public ActionResult LABasicAdd()
        {
            ...
            return View("~/Views/Regist/etc/AC Yard/LA/LABasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult LABasicAdd(LABasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (LA_CODE 생성 등)
            LABasicInfoRepository repo = new LABasicInfoRepository();
            string latestCode = repo.GetLatestLACode();
            model.LA_Code = GenerateNextLACode(latestCode, "LA");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateLABasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextLACode(string latestCode, string prefix)
        {
            int codeNumber = 0;
