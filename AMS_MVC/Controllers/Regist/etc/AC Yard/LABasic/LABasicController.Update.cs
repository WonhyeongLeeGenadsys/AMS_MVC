...
namespace AMS_MVC
{
    public partial class LABasicController : Controller
    {
        ...
        [HttpGet]
        public ActionResult UpdateLABasic(string tblIdx)
        {
            var model = laBasicRepository.GetLABasicInfoByTblIdxRepo(tblIdx);
            if (model == null)
            {
            ...
            return View("~/Views/Regist/etc/AC Yard/LA/LABasicUpdate.cshtml", model);
        }

        // POST: 수정 요청 처리
        [HttpPost]
        public ActionResult UpdateLABasicInfo(LABasicInfo model, HttpPostedFileBase photo)
        {
            ...
            var result = laBasicRepository.UpdateLABasicInfoRepo(model);
