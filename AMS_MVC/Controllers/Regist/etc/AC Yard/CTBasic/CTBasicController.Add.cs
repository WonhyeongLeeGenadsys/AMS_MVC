...
namespace AMS_MVC.Controllers
{
    ...
    {
        [HttpGet]
        public ActionResult CTBasicAdd()
        {
            ...
            return View("~/Views/Regist/etc/AC Yard/CT/CTBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult CTBasicAdd(CTBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (CT_CODE 생성 등)
            CTBasicInfoRepository repo = new CTBasicInfoRepository();
            string latestCode = repo.GetLatestCTCode();
            model.CT_Code = GenerateNextCTCode(latestCode, "CT");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreateCTBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextCTCode(string latestCode, string prefix)
        {
            int codeNumber = 0;
