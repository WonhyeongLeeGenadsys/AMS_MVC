...
namespace AMS_MVC
{
    public partial class CTBasicController : Controller
    {
        ...
        [HttpGet]
        public ActionResult UpdateCTBasic(string tblIdx)
        {
            var model = vcbBasicRepository.GetCTBasicInfoByTblIdxRepo(tblIdx);
            if (model == null)
            {
            ...
            return View("~/Views/Regist/etc/AC Yard/CT/CTBasicUpdate.cshtml", model);
        }

        // POST: 수정 요청 처리
        [HttpPost]
        public ActionResult UpdateCTBasicInfo(CTBasicInfo model, HttpPostedFileBase photo)
        {
            ...
            var result = vcbBasicRepository.UpdateCTBasicInfoRepo(model);
