...
namespace AMS_MVC
{
    public partial class PTBasicController : Controller
    {
        ...
        [HttpGet]
        public ActionResult UpdatePTBasic(string tblIdx)
        {
            var model = ptBasicRepository.GetPTBasicInfoByTblIdxRepo(tblIdx);
            if (model == null)
            {
            ...
            return View("~/Views/Regist/etc/AC Yard/PT/PTBasicUpdate.cshtml", model);
        }

        // POST: 수정 요청 처리
        [HttpPost]
        public ActionResult UpdatePTBasicInfo(PTBasicInfo model, HttpPostedFileBase photo)
        {
            ...
            var result = ptBasicRepository.UpdatePTBasicInfoRepo(model);
