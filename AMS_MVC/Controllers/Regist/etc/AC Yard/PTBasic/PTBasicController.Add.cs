...
namespace AMS_MVC
{
    ...
    {
        [HttpGet]
        public ActionResult PTBasicAdd()
        {
            ...
            return View("~/Views/Regist/etc/AC Yard/PT/PTBasicAdd.cshtml");
        }

        [HttpPost]
        public ActionResult PTBasicAdd(PTBasicInfo model, HttpPostedFileBase photo)
        {
            // 이미지 업로드 처리 
            ...
            // 기타 데이터 처리 (PT_CODE 생성 등)
            PTBasicInfoRepository repo = new PTBasicInfoRepository();
            string latestCode = repo.GetLatestPTCode();
            model.PT_Code = GenerateNextPTCode(latestCode, "PT");
            model.Writer = Session["User_Name"] != null ? Session["User_Name"].ToString() : "Anonymous";
            var result = repo.CreatePTBasicInfoRepo(model);

            return Json(new { success = result.IsSuccess, message = result.Message });
        }

        private static string GenerateNextPTCode(string latestCode, string prefix)
        {
            int codeNumber = 0;
