...
{
    public class HEATEXCHANGERDeviceController : Controller
    {
        ...
        private readonly HEATEXCHANGERChkRepository _heatexchangerChkRepo = new HEATEXCHANGERChkRepository();

        // HEATEXCHANGERBasicInfoRepository 사용
        private readonly HEATEXCHANGERBasicInfoRepository _heatexchangerBasicInfoRepo = new HEATEXCHANGERBasicInfoRepository();

        // HEATEXCHANGERDeviceInfo 페이지
        public ActionResult Index()
        {
            ViewBag.MenuType = "DeviceInfo";
            return View("~/Views/Device/etc/Cooling System/HEATEXCHANGER/HEATEXCHANGERDevice.cshtml");
        }

        // HEATEXCHANGERDeviceController (또는 HEATEXCHANGERDeviceDetailController)에 추가
        [HttpPost]
        ...
        /// </summary>
        [HttpPost]
        public JsonResult GetPriorityHEATEXCHANGER()
        {
            try
            {
                var priorityData = _priorityRepo.GetPriority(
                    "HEATEXCHANGER_BASICINFO", // HEATEXCHANGER 기본정보 테이블
                    "HEATEXCHANGER_CODE",      // HEATEXCHANGER 코드 필드
                    "HEATEXCHANGER",           // 표시용 장치 이름
                    "HEATEXCHANGER",           // 별칭
                    "AC"
                );
        ...
        /// <summary>
        /// 전체 HEATEXCHANGER의 월별 점검 데이터 가져오기 (JSON)
        /// </summary>
        public JsonResult GetMonthlyAllHEATEXCHANGERChkData()
        {
            try
            {
                var data = _heatexchangerChkRepo.GetMonthlyAllHEATEXCHANGERChkCounts();
        ...
        public JsonResult GetMonthlyMaintenanceData()
        {
            try
            {
                var data = _maintenanceRepo.GetMonthlyMaintenanceCounts("HEATEXCHANGER_MAINTENANCE_HISTORY", "HEATEXCHANGER");
                return Json(data, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// HEATEXCHANGER 단독 - 고장 테이블 정보 가져오기
        /// </summary>
        [HttpPost]
        public JsonResult GetGojangHEATEXCHANGERList()
        {
            try
            {
                // HEATEXCHANGER만 조회
                var gojangData = _gojangRepo.GetGojangData(
                    "HEATEXCHANGER_FAILURE_HISTORY", // 고장 이력 테이블
                    "HEATEXCHANGER_BASICINFO",       // 기본 정보 테이블
                    "HEATEXCHANGER_CODE",            // 매칭할 컬럼명
                    "HEATEXCHANGER",                 // 별칭
                    "HEATEXCHANGER"                  // EntityName (Grid에 표시용)
                );
        ...
        /// <summary>
        /// 기본정보 (HEATEXCHANGER 설비 목록) 데이터 가져오기
        /// </summary>
        public JsonResult GetBasicInfoList()
        {
            try
            {
                ...
                var result = _heatexchangerBasicInfoRepo.GetAllHEATEXCHANGERBasicInfoWithRiskMatrixRepo(out infoWithRisk);

                var formatted = infoWithRisk.Select(b => new
                {
                    HEATEXCHANGER_Code = b.HEATEXCHANGER_Code,
