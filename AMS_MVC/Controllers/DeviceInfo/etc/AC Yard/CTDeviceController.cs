...
{
    public class CTDeviceController : Controller
    {
        ...
        private readonly CTChkRepository _ctChkRepo = new CTChkRepository();

        // CTBasicInfoRepository 사용
        private readonly CTBasicInfoRepository _ctBasicInfoRepo = new CTBasicInfoRepository();

        // CTDeviceInfo 페이지
        public ActionResult Index()
        {
            ViewBag.MenuType = "DeviceInfo";
            return View("~/Views/Device/etc/AC Yard/CT/CTDevice.cshtml");
        }

        // CTDeviceController (또는 CTDeviceDetailController)에 추가
        [HttpPost]
        ...
        /// </summary>
        [HttpPost]
        public JsonResult GetPriorityCT()
        {
            try
            {
                var priorityData = _priorityRepo.GetPriority(
                    "CT_BASICINFO", // CT 기본정보 테이블
                    "CT_CODE",      // CT 코드 필드
                    "CT",           // 표시용 장치 이름
                    "CT",           // 별칭
                    "AC"
        ...
        /// <summary>
        /// 전체 CT의 월별 점검 데이터 가져오기 (JSON)
        /// </summary>
        public JsonResult GetMonthlyAllCTChkData()
        {
            try
            {
                var data = _ctChkRepo.GetMonthlyAllCTChkCounts();
        ...
        public JsonResult GetMonthlyMaintenanceData()
        {
            try
            {
                var data = _maintenanceRepo.GetMonthlyMaintenanceCounts("CT_MAINTENANCE_HISTORY", "CT");
                return Json(data, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// CT 단독 - 고장 테이블 정보 가져오기
        /// </summary>
        [HttpPost]
        public JsonResult GetGojangCTList()
        {
            try
            {
                // CT만 조회
                var gojangData = _gojangRepo.GetGojangData(
                    "CT_FAILURE_HISTORY", // 고장 이력 테이블
        ...
        public JsonResult GetBasicInfoList()
        {
            try
            {
                ...
                var result = _ctBasicInfoRepo.GetAllCTBasicInfoWithRiskMatrixRepo(out infoWithRisk);
