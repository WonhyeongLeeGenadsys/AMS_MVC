...
{
    public class DCCTDeviceController : Controller
    {
        ...
        private readonly DCCTChkRepository _dcctChkRepo = new DCCTChkRepository();

        // DCCTBasicInfoRepository 사용
        private readonly DCCTBasicInfoRepository _dcctBasicInfoRepo = new DCCTBasicInfoRepository();

        // DCCTDeviceInfo 페이지
        public ActionResult Index()
        {
            ViewBag.MenuType = "DeviceInfo";
            return View("~/Views/Device/etc/DC Yard/DCCT/DCCTDevice.cshtml");
        }

        // DCCTDeviceController (또는 DCCTDeviceDetailController)에 추가
        [HttpPost]
        ...
        /// </summary>
        [HttpPost]
        public JsonResult GetPriorityDCCT()
        {
            try
            {
                var priorityData = _priorityRepo.GetPriority(
                    "DCCT_BASICINFO", // DCCT 기본정보 테이블
                    "DCCT_CODE",      // DCCT 코드 필드
                    "DCCT",           // 표시용 장치 이름
                    "DCCT",           // 별칭
                    "AC"
                );
        ...
        /// <summary>
        /// 전체 DCCT의 월별 점검 데이터 가져오기 (JSON)
        /// </summary>
        public JsonResult GetMonthlyAllDCCTChkData()
        {
            try
            {
                var data = _dcctChkRepo.GetMonthlyAllDCCTChkCounts();
        ...
        public JsonResult GetMonthlyMaintenanceData()
        {
            try
            {
                var data = _maintenanceRepo.GetMonthlyMaintenanceCounts("DCCT_MAINTENANCE_HISTORY", "DCCT");
                return Json(data, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// DCCT 단독 - 고장 테이블 정보 가져오기
        /// </summary>
        [HttpPost]
        public JsonResult GetGojangDCCTList()
        {
            try
            {
                // DCCT만 조회
                var gojangData = _gojangRepo.GetGojangData(
                    "DCCT_FAILURE_HISTORY", // 고장 이력 테이블
                    "DCCT_BASICINFO",       // 기본 정보 테이블
                    "DCCT_CODE",            // 매칭할 컬럼명
                    "DCCT",                 // 별칭
                    "DCCT"                  // EntityName (Grid에 표시용)
                );
        ...
        public JsonResult GetBasicInfoList()
        {
            try
            {
                ...
                var result = _dcctBasicInfoRepo.GetAllDCCTBasicInfoWithRiskMatrixRepo(out infoWithRisk);

                var formatted = infoWithRisk.Select(b => new
                {
                    DCCT_Code = b.DCCT_Code,
