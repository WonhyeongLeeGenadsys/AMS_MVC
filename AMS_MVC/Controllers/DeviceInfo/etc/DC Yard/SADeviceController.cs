...
{
    public class SADeviceController : Controller
    {
        ...
        private readonly SAChkRepository _saChkRepo = new SAChkRepository();

        // SABasicInfoRepository 사용
        private readonly SABasicInfoRepository _saBasicInfoRepo = new SABasicInfoRepository();

        // SADeviceInfo 페이지
        public ActionResult Index()
        {
            ViewBag.MenuType = "DeviceInfo";
            return View("~/Views/Device/etc/DC Yard/SA/SADevice.cshtml");
        }

        // SADeviceController (또는 SADeviceDetailController)에 추가
        [HttpPost]
        ...
        /// </summary>
        [HttpPost]
        public JsonResult GetPrioritySA()
        {
            try
            {
                var priorityData = _priorityRepo.GetPriority(
                    "SA_BASICINFO", // SA 기본정보 테이블
                    "SA_CODE",      // SA 코드 필드
                    "SA",           // 표시용 장치 이름
                    "SA",           // 별칭
                    "DC"
        ...
        /// <summary>
        /// 전체 SA의 월별 점검 데이터 가져오기 (JSON)
        /// </summary>
        public JsonResult GetMonthlyAllSAChkData()
        {
            try
            {
                var data = _saChkRepo.GetMonthlyAllSAChkCounts();
        ...
        public JsonResult GetMonthlyMaintenanceData()
        {
            try
            {
                var data = _maintenanceRepo.GetMonthlyMaintenanceCounts("SA_MAINTENANCE_HISTORY", "SA");
                return Json(data, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// SA 단독 - 고장 테이블 정보 가져오기
        /// </summary>
        [HttpPost]
        public JsonResult GetGojangSAList()
        {
            try
            {
                // SA만 조회
                var gojangData = _gojangRepo.GetGojangData(
                    "SA_FAILURE_HISTORY", // 고장 이력 테이블
                    "SA_BASICINFO",       // 기본 정보 테이블
                    "SA_CODE",            // 매칭할 컬럼명
                    "SA",                 // 별칭
                    "SA"                  // EntityName (Grid에 표시용)
                );
        ...
        public JsonResult GetBasicInfoList()
        {
            try
            {
                ...
                var result = _saBasicInfoRepo.GetAllSABasicInfoWithRiskMatrixRepo(out infoWithRisk);
