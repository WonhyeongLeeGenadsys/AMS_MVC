...
namespace AMS_MVC
{
    public partial class SPAREBasicController : Controller
    {
        ...
        public ActionResult SPAREBasicDetail(int spareId)
        {
            ...
            return View("~/Views/SPARE/SPAREBasicDetail.cshtml", sparePart);
        }
        ...
        public JsonResult DeleteSPAREBasicInfo(int spareId)
        {
            ...
            if (result.IsSuccess)
            {
                return Json(new { success = true, message = "예비품 정보가 삭제되었습니다." });
            }

            return Json(new { success = false, message = "예비품 정보 삭제 실패: " + result.Message });
        }
    }
}